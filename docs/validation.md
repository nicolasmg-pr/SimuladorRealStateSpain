...
## The rent cap's quantity leg is repaired, and G1 passes (2026-09-17)

**1 failed / 180 passed / 8 xfailed**, from 6 / 175 / 8 at the start of the day. The last step was
a model fix, not bookkeeping — an earlier attempt to close the two cap gates by marking them strict
xfail was correctly called out as tracking rather than repair, and is reverted.

**The missing friction: IRPF on the realised gain.** `exit_cost_for` charged only the transaction
cost of selling. Spain also charges 19–28% of the gain. On an appreciated dwelling that is several
times the transaction cost, and the model charged a landlord selling a doubled asset exactly what
it charged one selling at cost. It had been excluded deliberately on two reasons, **both wrong**:
`r_req` nets *expected future* growth while IRPF taxes the *realised past* gain, and the per-unit
basis it said was missing is `Unit.last_sale_price`, set on every transaction.

The test applied before reversing a deliberate exclusion — *would this be done if the gate were
passing?* — it would, because the cost of leaving is understated without it whatever any gate
reads.

| ten seeds, Ley 11/2020 | before | after | sourced |
|---|---|---|---|
| new leases | −47.5% | **−17.0%** | **−10% to −20%** |
| co-movement ratio | 4.272 | **0.809** | **0.07–3.2** |

**G1 passes.** §7.2b's declared falsification, red since it was written, closes on a mechanism the
model was missing rather than on a parameter moved inside a range.

**What remains is the price leg, and its cause is measured.** Rent reads −20.6% against a −3% to
−7% band. It is the cap's own mechanical bite: `cap_reference_discount` declares the index sits 5%
below prevailing market rent (range 0–10%) and the **realised** discount at activation is
**−20.7%** over five seeds. The reference is an EWMA at weight 0.1, mean lag `(1−w)/w` = **9
quarters**, so it drifts far below the index it shadows. **Rent falls by exactly the realised
discount** — −20.6% against −20.7%.

**A correction was attempted and reverted, and the attempt is the useful part.** The published
Catalan index is a twelve-month trailing mean lagging ≈1.5 quarters, so `(1−w)/w = 1.5` gives
w = 0.4 — derived, not chosen. It moved rent to −10.9% and leases to −8.4% and **broke G3 and the
supply-response gate**, 1 failed → 3. Reverted. The declared-versus-realised mismatch is a
spec–code disagreement and is the next repair, but the withdrawal timing has to be re-derived with
it rather than an EWMA weight changed underneath it.

**Two candidate repairs were tested and refuted along the way**, and both are recorded so they are
not retried: the outside-option comparison in the reservation rent (a no-op — the existing
